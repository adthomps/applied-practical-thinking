...
export default function InsightDetail() {
  ...
  const detailQuery = useFeedDetailQuery("insights", id);
  const insight = detailQuery.data?.item || null;
  const markdown = detailQuery.data?.markdown || "";
  const loading = detailQuery.isLoading;
  const error = detailQuery.error?.message || (!loading && !insight ? "Insight not found" : null);
  ...
  const insightItem = toContentIndexItemFromFeed(insight);
  const media = (insightItem as any).media as
    | { embedUrl?: string; audioUrl?: string; videoUrl?: string }
    | undefined;
  const TypeIcon = typeIcons[insightItem.type] || Book;

  const mediaBlock =
    insightItem.type === "podcast" && media ? (
      <div className="p-6 bg-muted/30 rounded-lg border border-border">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">{insightItem.duration}</p>
          </div>
        </div>
  ...
  return (
    <ContentDetailPage
      ...
      markdown={markdown}
      aboutTitle="Summary"
      headerMeta={<InsightMeta insight={insightItem} showType={false} showDate={false} showConcepts={false} />}
      heroFallback={
        <div className="flex flex-col items-center gap-2 text-muted-foreground/50">
          <TypeIcon className="h-12 w-12" />
          <span className="text-sm">{typeLabels[insightItem.type]}</span>
        </div>
      }
      mainTop={mediaBlock}
    />
